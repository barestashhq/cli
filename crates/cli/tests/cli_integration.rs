...
async fn events_stream_emits_only_jsonl_and_ctrl_c_exits_successfully() {
    ...
    process
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
    let mut child = process.spawn().expect("spawn events stream");
    let stdout = child.stdout.take().expect("capture events stream stdout");
    ...
    let stdout_reader = thread::spawn(move || {
        ...
        let bytes = reader
            ...
            .expect("read first stream output line");
        first_line_tx
            .send(bytes > 0)
            .expect("report first stream output line");
        reader
    ...
    let emitted_jsonl = first_line_rx
        .recv_timeout(Duration::from_secs(5))
        .unwrap_or(false);
    if !emitted_jsonl {
        let _ = child.kill();
        let _ = child.wait();
        panic!("stream did not emit JSONL before the timeout");
    }
    ...
    let stdout = stdout_reader.join().expect("join stream stdout reader");
