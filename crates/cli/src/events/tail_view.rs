...
use crate::output::{
    ...
    sanitize_terminal_text, visible_width,
};
...
impl TailView {
    ...
    pub fn add(&mut self, event: EventMetadata) -> Result<(), PresentationError> {
        self.started = true;
    ...
    pub fn stop(&mut self) -> Result<(), PresentationError> {
        let stdout = io::stdout();
        let mut output = stdout.lock();
        self.stop_to(&mut output)
    }

    fn stop_to(&mut self, output: &mut impl Write) -> Result<(), PresentationError> {
    ...
    fn render(&self) -> Result<(), PresentationError> {
